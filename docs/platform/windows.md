...
请尝试在启动器设置中将下载源设置为 `Cafe 下载源`（Cafe Launcher），或在启动器设置中点击 `修复` 以检查游戏资源完整性。
